    ...
    if (command === "play") {
      ...
      try {
        ...
        if (!apiData.download_url) {
          await sendCustomReaction(client, message, "❌");
          return await client.sendMessage(message.from, {
            text: "*𝐓𝐊𝐓-𝐂𝐘𝐁𝐄𝐑-𝐀𝐈* " + toFancyFont("No download URL available"),
            mentions: [message.sender]
          }, { quoted: message });
        }
        ...
        const footer = (typeof config !== 'undefined' && config.FOOTER) || "> ᴍᴀᴅᴇ ᴡɪᴛʜ 🤍 ʙʏ 𝐓𝐊𝐓-𝐓𝐄𝐂𝐇🇿🇼";
      ...
      try {
        ...
        await client.sendMessage(message.from, {
          text: "*𝐓𝐊𝐓-𝐂𝐘𝐁𝐄𝐑-𝐀𝐈* " + toFancyFont(`failed to process ${command} file`),
          mentions: [message.sender]
        }, { quoted: message });
